...
import { BatchBanDialog } from "./components/BatchBanDialog";
    ...
    <TooltipProvider delayDuration={150}>
      <main className="relative z-10 min-h-[calc(100vh-3.5rem)] bg-background lg:min-h-[calc(100vh-4rem)]">
        <div className="flex min-h-[calc(100vh-3.5rem)] lg:min-h-[calc(100vh-4rem)]">
          <AdminSidebar collapsed={sidebarCollapsed} items={visibleSectionItems} section={section} onCollapsedChange={setSidebarCollapsed} onSection={setSection} />
          <section className={`min-w-0 flex-1 space-y-6 px-4 py-6 transition-[margin] lg:px-6 ${sidebarCollapsed ? "lg:ml-20" : "lg:ml-64"}`}>
            ...
            {section === "users" || section === "banned" ? (
              <UsersSection mode={section} bannedStatusFilter={users.bannedStatusFilter} onBannedStatusFilter={users.setBannedStatusFilter} onPage={users.setPage} onPageSize={users.setPageSize} onRefresh={users.refetchUsers} onSearch={users.setSearch} onSort={users.setSort} page={users.page} pageCount={users.pageCount} pageSize={users.pageSize} roles={catalogs.roles} search={users.search} sort={users.sort} totalUsers={users.totalUsers} userColumns={userColumns} userFilters={users.userFilters} users={users.users} usersError={users.usersError} usersLoading={users.usersLoading} onApplyUserFilters={users.applyUserFilters} onResetUserFilters={users.resetUserFilters} />
            ) : null}
            ...
            {section === "emails" ? <EmailSection blockedDomains={email.blockedDomains} blockedDomainsDraft={email.blockedDomainsDraft} canUpdateEmails={permissions.canUpdateEmails} dailyIpLimit={email.dailyIpLimit} dailyRecipientLimit={email.dailyRecipientLimit} dailyResetHourUtc={email.dailyResetHourUtc} emailLimits={email.emailLimits} emailLimitsError={email.emailLimitsError} emailLimitsLoading={email.emailLimitsLoading} emailLimitsSaving={email.emailLimitsSaving} monthlyHardLimit={email.monthlyHardLimit} monthlyResetDay={email.monthlyResetDay} onBlockedDomainsDraft={email.setBlockedDomainsDraft} onDailyIpLimit={email.setDailyIpLimit} onDailyRecipientLimit={email.setDailyRecipientLimit} onDailyResetHourUtc={email.setDailyResetHourUtc} onMonthlyHardLimit={email.setMonthlyHardLimit} onMonthlyResetDay={email.setMonthlyResetDay} onRefresh={email.refetchEmailLimits} onSave={() => void email.saveEmailLimits()} onSaveBlockedDomains={() => void email.saveBlockedDomains()} /> : null}
            ...
            {section === "credits" ? <CreditsSection canReadUsers={permissions.canReadUsers} canUpdate={permissions.canUpdateCredits} draft={credits.draft} error={credits.error} loading={credits.loading} saving={credits.saving} userResults={credits.userResults} userSearch={credits.userSearch} userSearchLoading={credits.userSearchLoading} onAddCategory={credits.addCategory} onAddGroup={credits.addGroup} onDeleteCategory={credits.removeCategory} onDeleteGroup={credits.deleteGroup} onDraft={credits.setDraft} onMoveCategory={credits.moveCategory} onMoveGroup={credits.moveGroup} onRefresh={credits.refresh} onReorderCategory={credits.reorderCategory} onReorderGroup={credits.reorderGroup} onSave={() => void credits.save()} onSearchUsers={() => void credits.searchUsers()} onSetCategoryDescription={credits.setCategoryDescription} onSetCategoryName={credits.setCategoryName} onSetGroupDescription={credits.setGroupDescription} onSetGroupTitle={credits.setGroupTitle} onSetUserSearch={credits.setUserSearch} /> : null}
        ...
        <BatchBanDialog actionLoading={actionLoading} reason={accountActions.batchBanReason} user={accountActions.batchBanUser} onReason={accountActions.setBatchBanReason} onSave={() => void accountActions.saveBatchBan()} onUser={accountActions.setBatchBanUser} />
